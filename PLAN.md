...
- **Optimización y asignación automática de rutas** — usar un algoritmo determinista de Vehicle Routing Problem (VRP), no un LLM. Primera UX planeada: "Sugerir distribución" (el sistema propone asignaciones de chofer, orden de paradas, distancia/tiempo estimado) seguido de "Aplicar distribución" tras revisión del manager; la asignación manual permanece disponible siempre — la automatización es aditiva, nunca un reemplazo. Fases posteriores podrían agregar rebalanceo automático/dinámico.
