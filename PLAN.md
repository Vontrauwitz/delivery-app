...
**Funcionalidad de ubicación/mapa.**

Alertas (antes documentado en esta sección) ya está implementado — ver "IMPLEMENTADO / CERRADO" arriba. Con ese checkpoint cerrado, el siguiente paso tomado del backlog es completar la funcionalidad de ubicación/mapa, aún sin empezar:

- Pantalla de mapa en vivo para el manager (posición actual de cada chofer, ya con la semántica de frescura/`isStale` existente en `locations`).
- Vista de historial de recorrido — `LocationPing` ya guarda cada ping, no solo el último, así que esto es una consulta nueva, no un cambio de modelo (ver "Puntos de extensión previstos" más abajo).
- Optimización de rutas, como módulo nuevo que solo lee `locations`/`dispatch` sin modificarlos.
- Conexión natural con la alerta `LOCATION_STALE` recién implementada: la misma noción de frescura que dispara esa alerta es la que un mapa en vivo visualizaría directamente.
...
Preservado explícitamente, no descartado, no iniciado. (La funcionalidad de ubicación/mapa se movió a "SIGUIENTE FASE PLANIFICADA" arriba — sigue sin iniciarse, solo cambió de sección.)
...
- **Pulido de UI/UX** — heredado del alcance original de la Fase 5; buena parte del pulido ya se aplicó de forma incremental (dashboard de administración, Programación, gestión de choferes, Configuración, Mensajería/Dispatch, Alertas), pero algunas pantallas más antiguas (ej. `weekly-report`, `accounting-periods`, `promotions`) no se han revisado con el sistema visual actual.
