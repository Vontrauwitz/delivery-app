...
// WEEKLY is manager/admin-only for this phase: it's a vehicle-level audit not tied to a
// session/shift, so it doesn't fit the existing driver-authorization chain (which always
// resolves "my own active session" via vehicle -> shift -> session). Extending that chain to
// a session-less action would be a new permission shape, not a reuse of the existing one — see
// PLAN deviations in the Phase 3 summary for the full rationale.
